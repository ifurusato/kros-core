...
class QueuePublisher(Publisher):
    ...
    def put(self, message):
        if not self.is_active:
            ...
            self._log.info('🍓 put message into queue: {} ({:d} items)'.format(message, self._queue.size))
        ...
        while f_is_enabled():
            ...
            if not self.suppressed:
                while not self._queue.empty():
                    ...
                    self._log.info(Fore.MAGENTA + '[{:03d}] published message: {}; event: {} '.format(
                             _count, _message.name, _message.event.label) + Fore.WHITE
                             + ' with value: {}'.format(_message.payload.value))
