...
class Roam(Behaviour):
    ...
    def _set_max_fwd_velocity_by_distance(self, distance_cm):
        ...
        Both motors share the same limit, as there's no reason to have them
        be different. Because the values are stored on each motor separately
        we'll just use the PORT motor for calculations.
        '''
...
#       if self._port_motor.velocity > 0.0 and self._stbd_motor.velocity > 0.0:
        ...
        if distance_cm >= self._max_distance: 
            self._log.info(Fore.YELLOW + '💊😎 no speed limit:  {}')
            ...
            self._log.info(Fore.GREEN + '💊😀 setting speed limit...')
        ...
        self._log.info('🍍 motors max velocity: {:5.2f}; max fwd: {:5.2f}'.format( _motors_max_fwd_velocity, _roam_max_velocity))
        ...
        if _roam_max_velocity < _motors_max_fwd_velocity:
            ...
            self._log.info('🍏 set max forward velocity limit for both motors to: {:5.2f}'.format(_roam_max_velocity))
        else:
            self._log.info('🍎 did not set speed.')
